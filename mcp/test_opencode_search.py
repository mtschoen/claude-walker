...
def test_case_sensitivity_and_tool_blocks(opencode_db):
    ...
    assert with_tools["hits"][0]["source_id"] == "p5"
    assert with_tools["hits"][0]["line_number"] == 5
...
@pytest.mark.parametrize("field", ["message_data", "part_data"])
@pytest.mark.parametrize("value", [None, [], "valid JSON scalar"])
def test_valid_non_object_json_rows_are_skipped(field, value):
    row = {
        "part_rowid": 1,
        "part_id": "p_scalar",
        "message_id": "m_scalar",
        "session_id": "ses_scalar",
        "time_created": 1_730_000_000_000,
        "message_data": _json({"role": "user"}),
        "part_data": _json({"type": "text", "text": "must not crash"}),
        "directory": "/work/scalar",
        "slug": "scalar-session",
        "title": "Scalar session",
    }
    row[field] = _json(value)
    ...
    assert opencode_search._load_messages(Connection(), include_tools=False) == []
