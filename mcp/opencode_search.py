...
class _Message:
    ...
    session_slug: str
    session_title: str
    text_parts: list[_PartSegment] = field(default_factory=list)
    tool_parts: list[_PartSegment] = field(default_factory=list)

    @property
    def text(self) -> str:
        return "\n".join(part.text for part in self.text_parts)

    @property
    def searchable_parts(self) -> tuple[_PartSegment, ...]:
        return (*self.text_parts, *self.tool_parts)

    @property
    def searchable_text(self) -> str:
        return "\n".join(part.text for part in self.searchable_parts)

    def source_for_offset(self, offset: int) -> _PartSegment:
        parts = self.searchable_parts
        while offset > len(parts[0].text):
            offset -= len(parts[0].text) + 1
            parts = parts[1:]
        return parts[0]
...
def _load_messages(
    ...
    for row in rows:
        try:
            ...
            continue
        if not isinstance(message_data, dict) or not isinstance(part_data, dict):
            continue
        ...
        if part_type == "text" and isinstance(part_data.get("text"), str):
            message.text_parts.append(
                _PartSegment(row["part_rowid"], row["part_id"], part_data["text"])
            )
        elif part_type == "tool":
            text = _tool_text(part_data)
            if text:
                message.tool_parts.append(
                    _PartSegment(row["part_rowid"], row["part_id"], text)
                )
    ...
    for session_messages in sessions.values():
        for index, message in enumerate(session_messages):
            ...
            if not text or not matches:
                continue
            source = message.source_for_offset(matches[0].start())
            ...
            hits.append(
                {
                    ...
                    "source_id": source.part_id,
                    "line_number": source.rowid,
                    "timestamp": _timestamp_string(message.timestamp_ms),
                    "role": message.role,
                    "snippet": snippet,
